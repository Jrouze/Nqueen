// -----------------------------------------------------------------------------
//    Nqueens problem |  OneAPI version using nd_range and USM, range on CPU
// -----------------------------------------------------------------------------
...
void BP_queens_root_dfs_cpu(item<1> &item, int N, unsigned int nPreFixos, int depthPreFixos,
  const QueenRoot *__restrict root_prefixes, unsigned long long *__restrict vector_of_tree_size,
  unsigned long long *__restrict sols) {
  int idx = item[0];
  if (idx < nPreFixos) {
...
void nqueens(short size, int initial_depth, unsigned int n_explorers, QueenRoot *root_prefixes_h ,
  ...
  if (device==0) { // ------------ If work done on GPU ------------
    ...
    for (int i = 0; i < repeat; i++)
     ...
     for (int i = 0; i < repeat; i++)
       Qh.parallel_for(gws, [=] (item<1> item) {
           BP_queens_root_dfs_cpu(item, size, n_explorers, initial_depth, root_prefixes_h,
                              vector_of_tree_size_h, sols_h);
      });
    Qh.wait_and_throw();
  }
    ...
    for (int i = 0; i < repeat; i++){
      Qd.parallel_for(nd_range<1>(gws_D, lws), [=] (nd_item<1> item) {
          BP_queens_root_dfs(item, size, n_explorers, initial_depth, root_prefixes_h,
                             vector_of_tree_size_h, sols_h);
      });
      Qh.parallel_for(gws_H, [=] (item<1> item) {
          BP_queens_root_dfs_cpu(item, size, n_explorers, initial_depth, root_prefixes_h+gpupart,
